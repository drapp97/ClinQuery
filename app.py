...
st.title("🧬 ClinQuery")
st.write("Search ClinVar variants by gene. Optional HGVS cDNA filter included.")
...
if st.sidebar.button("Search"):
    if not gene:
        ...
        try:
            ...
            if not id_list:
                st.warning(f"No results found for gene {gene}.")
            else:
                ...
                for clinvar_record in root.findall(".//ClinVarSet"):
                    # Safely get basic fields
                    rcv_id = clinvar_record.findtext("ClinVarAccession/Acc") or "N/A"
                    significance = clinvar_record.findtext("ReferenceClinVarAssertion/ClinicalSignificance/Description") or "N/A"
                    ...
                    # Collect all HGVS strings if present
                    hgvs_c = []
                    hgvs_p = []
                ...
                # Optional: filter locally if user entered variant
                if variant:
